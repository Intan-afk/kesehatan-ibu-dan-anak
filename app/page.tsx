...
export default function MobileKIAppFinal() {
  ...
  // NAMA UTAMA: Awalnya kosong agar benar-benar mengikuti input login
  const [namaBunda, setNamaBunda] = useState("");
  const [tempInput, setTempInput] = useState(""); 

  const handleLogin = (e: { preventDefault: () => void; }) => {
    e.preventDefault();
    // Jika kotak username kosong, kita beri nama "Bunda", jika diisi "Intan", maka "Intan" yang dipakai.
    const namaUser = tempInput.trim() !== "" ? tempInput : "Bunda";
    setNamaBunda(namaUser);
    setIsLoggedIn(true);
  };
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
            ...
            <Input 
              ...
              className="bg-white rounded-full border-slate-200 h-12 px-6 focus:ring-pink-300 font-bold"
            />
          </div>
          ...
          <button 
            onClick={() => { 
              ...
              setTempInput(""); // Reset input saat logout
            }}
            className="w-full flex items-center gap-4 p-4 bg-red-50 hover:bg-red-100 rounded-2xl transition-all text-red-600 font-black"
          >
            <span>🚪</span> Logout & Ganti Nama
          </button>
        </div>

        <Button 
          onClick={() => setShowProfileMenu(false)}
          className="w-full mt-8 rounded-full py-7 bg-pink-300 text-slate-800 font-black hover:bg-pink-400"
        >
          SIMPAN
        </Button>
      </div>
        ...
        <div className="w-full max-w-[380px] space-y-8">
          ...
          <form onSubmit={handleLogin} className="space-y-5 px-10">
            <div className="text-center space-y-2">
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Siapa nama Anda?</p>
              <Input 
                placeholder="Ketik Intan / Olive / Nadia" 
                value={tempInput}
                onChange={(e) => setTempInput(e.target.value)}
                className="rounded-full border-slate-200 h-14 text-center text-lg shadow-sm focus:ring-pink-300 font-bold" 
              />
            </div>
    ...
    <div className="min-h-screen bg-white relative font-sans overflow-x-hidden">
      ...
      <div className="max-w-2xl mx-auto pt-12 px-6 pb-32">
        ...
        {activeTab === "dashboard" ? (
          <div className="animate-in fade-in duration-700">
            {/* Nama di sini 100% otomatis ikut input login */}
            ...
            <div className="grid grid-cols-3 gap-4 mb-12">
              ...
              <button onClick={() => setActiveTab("riwayat_imunisasi")} className="bg-[#FBCFE8] border border-pink-200 rounded-[2.5rem] p-4 flex flex-col items-center gap-3 shadow-md active:scale-90 transition-all text-slate-800">
                <div className="w-14 h-14 bg-[#60A5FA] rounded-2xl flex items-center justify-center text-3xl text-white shadow-lg">💉</div>
                <p className="text-[10px] font-black uppercase text-center leading-tight">riwayat imunisasi</p>
              </button>
              <button onClick={() => setActiveTab("jadwal_imunisasi")} className="bg-[#FBCFE8] border border-pink-200 rounded-[2.5rem] p-4 flex flex-col items-center gap-3 shadow-md active:scale-90 transition-all text-slate-800">
                <div className="w-14 h-14 bg-[#60A5FA] rounded-2xl flex items-center justify-center text-3xl text-white shadow-lg">📅</div>
                <p className="text-[10px] font-black uppercase text-center leading-tight">jadwal imunisasi</p>
              </button>
            </div>
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-[3.5rem] p-8 shadow-2xl border border-white min-h-[400px]">
             <button onClick={() => setActiveTab("dashboard")} className="bg-slate-100 text-slate-500 px-6 py-2 rounded-full font-black text-xs mb-8">
               ← KEMBALI
             </button>
             <h2 className="text-2xl font-black text-slate-800">Halaman {activeTab.replace("_", " ")}</h2>
             <p className="mt-4 text-slate-500">Menampilkan data spesifik untuk <strong>Bunda {namaBunda}</strong>.</p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 flex justify-center p-6 z-40">
        <div className="w-full max-w-md bg-white/80 backdrop-blur-2xl border border-white/50 p-4 flex justify-around items-center rounded-[3rem] shadow-2xl">
           <span onClick={() => {setActiveTab("dashboard"); setShowProfileMenu(false);}} className={`text-3xl cursor-pointer ${activeTab === 'dashboard' ? 'text-pink-400' : 'text-slate-200'}`}>🏠</span>
